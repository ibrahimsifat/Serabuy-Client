...
import NavCategoryCard from '@component/NavberCategory/NavCategoryCard';
...
const Category = () => {
  ...
  return (
    <div className="flex flex-col w-full h-full bg-white cursor-pointer scrollbar-hide">
      {categoryDrawerOpen && (
        <div className="w-full flex justify-between items-center h-16 px-6 py-4 bg_green text-white border-b border-gray-100 bg-">
      ...
      <div className="overflow-y-scroll scrollbar-hide w-full max-h-full bg-gray-100">
        {categoryDrawerOpen && (
        ...
        {error ? (
          ...
          <div className="relative grid lg:grid-cols-6 md:grid-cols-6 gap-2 justify-center items-center py-10 px-6">
            {data?.map((category) => (
              <NavCategoryCard
                key={category._id}
              ...
              {pages.map((item) => (
                ...
                >
                  <item.icon
                    ...
                    aria-hidden="true"
                  />
                  <p className="inline-flex items-center justify-between ml-2 text-sm font-medium w-full hover:text-green-600 text-black">
                    {item.title}
                  </p>
                </a>
              ))}
